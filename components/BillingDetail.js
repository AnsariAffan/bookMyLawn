...
const [receviedAmount,setReceviedAmount] = useState(paidAmount+bookAmount)
  ...
  useEffect(() => {
    ...
    setOpenAmount(calculatedOpenAmount) // Set the remaining amount with two decimal places

    // Update payment status based on paid amount
    const totalAmount = billingDetails.finalAmount;
    const paid = paidAmount 

    if (Number(paidAmount) + Number(bookAmount) === 0) {
      setpaymentStatus("Not Paid");
    } else if (Number(paidAmount) + Number(bookAmount) === totalAmount) {
      setpaymentStatus("Fully Paid");
    } else if (Number(paidAmount) + Number(bookAmount) === totalAmount / 2) {
      setpaymentStatus("Half Paid");
    } else if (Number(paidAmount) + Number(bookAmount) > 0 && Number(paidAmount) + Number(bookAmount) < totalAmount / 2) {
      setpaymentStatus("Partial Paid");
    } else {
      setpaymentStatus("Partial Paid"); // For any other amount
    }
  }, [paidAmount, billingDetails.finalAmount]);
